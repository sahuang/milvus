...
void
GpuIndex::search_without_codes(Index::idx_t n,
                 const float* x,
                 float* original_data,
                 Index::idx_t k,
                 float* distances,
  ...
  if (getDeviceForAddress(x) == -1) {
    ...
    if (dataSize >= minPagedSize_) {
      searchFromCpuPagedWithoutCodes_(n, x, original_data, k,
                          outDistances.data(),
                          outLabels.data(),
                          bitset);
      usePaged = true;
  ...
  if (!usePaged) {
    searchNonPagedWithoutCodes_(n, x, original_data, k,
                    outDistances.data(),
                    outLabels.data(),
                    bitset);
  }
...
void
GpuIndex::searchNonPagedWithoutCodes_(int n,
                          const float* x,
                          float* original_data,
                          int k,
                          float* outDistancesData,
  ...
  searchImplWithoutCodes_(n, vecs.data(), original_data, k, outDistancesData, outIndicesData, bitset);
}

void
GpuIndex::searchFromCpuPagedWithoutCodes_(int n,
                              const float* x,
                              float* original_data,
                              int k,
                              float* outDistancesData,
  ...
  if (!pinnedAlloc.first || pageSizeInVecs < 1) {
    ...
    for (int cur = 0; cur < n; cur += batchSize) {
      ...
      searchNonPagedWithoutCodes_(num,
                      x + (size_t) cur * this->d,
                      original_data,
                      k,
                      outDistancesSlice.data(),
  ...
  while (cur3 < n) {
    ...
    if (cur3 != -1 && cur3 < n) {
      ...
      searchImplWithoutCodes_(numToProcess,
                  bufGpus[cur3BufIndex]->data(),
                  original_data,
                  k,
                  outDistancesSlice.data(),
