...
MAX_NQ = 10001
FILE_PREFIX = "binary_"
SIFT_SRC_DATA_DIR = '/home/sift_data/'
SIFT_SRC_GROUNDTRUTH_DATA_DIR = SIFT_SRC_DATA_DIR + 'gnd'

_HOST = '127.0.0.1'
_PORT = '19530'  # default value

logger = logging.getLogger("milvus_test.runner")
...
collection_name = 'test_optimizer_collection'
...
for i in range(100):
    ...
    if vectors:
        ...
        res_ids = milvus.insert(collection_name, hybrid_entities, ids=ids)
        assert ids == res_ids
    ...
    query_hybrid = {
        "bool": {
            "must": [
                {
                    "range": {
                        "int_field": {"GT": -1, "LT": 990}
                    }
                },
                {
                    "vector": {
                        "vec_float": {
                            "topk": topk, "query": query_vectors, "params": {"nprobe": 16}
                        }
                    }
                }
            ],
