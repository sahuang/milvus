...
template <typename Codec, typename Metric>
__global__ void
ivfFlatScanWithoutCodes(Tensor<float, 2, true> queries,
            bool useResidual,
            Tensor<float, 3, true> residualBase,
            Tensor<int, 2, true> listIds,
  ...
  auto query = queries[queryId].data();
  auto indices = indicesData[listId];
  // auto vecs = allListData[listId];
  auto numVecs = listLengths[listId];
  auto dim = queries.getSize(1);
...
void
runIVFFlatScanTileWithoutCodes(Tensor<float, 2, true>& queries,
                   Tensor<int, 2, true>& listIds,
                   Tensor<uint8_t, 1, true>& bitset,
                   Tensor<float, 1, true>& originalData,
                   thrust::device_vector<void*>& listIndices,
...
void
runIVFFlatScanWithoutCodes(Tensor<float, 2, true>& queries,
               Tensor<int, 2, true>& listIds,
               Tensor<uint8_t, 1, true>& bitset,
               Tensor<float, 1, true>& originalData,
               thrust::device_vector<void*>& listIndices,
  ...
  for (int query = 0; query < queries.getSize(0); query += queryTileSize) {
    ...
    runIVFFlatScanTileWithoutCodes(queryView,
                       listIdsView,
                       bitset,
                       originalData,
                       listIndices,
