...
def main():
    ...
    collection_name = 'example_collection_' + "strategy_4"
    ...
    tag = ["red", "blue"]
    # milvus.create_partition(collection_name, tag[0])
    # milvus.create_partition(collection_name, tag[1])
    ...
    hybrid_entities_0 = [
        {"field": "int_field", "values": int_list[0 : rows // 2], "type": DataType.INT32},
        {"field": "vec", "values": vec[0 : (rows // 2)], "type": DataType.FLOAT_VECTOR}
    ]
    hybrid_entities_1 = [
        {"field": "int_field", "values": int_list[(rows // 2) + 1 : rows], "type": DataType.INT32},
        {"field": "vec", "values": vec[(rows // 2) + 1 : rows], "type": DataType.FLOAT_VECTOR}
    ]

    # ids = milvus.insert(collection_name, hybrid_entities_0, [i for i in range(rows // 2)], partition_tag=tag[0])
    # ids = milvus.insert(collection_name, hybrid_entities_1, [i for i in range((rows // 2) + 1 , rows)], partition_tag=tag[1])
    ...
    for strategy in [2, 2, 3, 1]:
        ...
        if passed:
            print("Start search ..")
        t0 = time.time()
        results = milvus.search(collection_name, query_hybrid, partition_tags=[tag[0]])
        if passed:
