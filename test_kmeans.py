...
try:
    ...
    for c in combinations:
        ...
        for loop in range(segments):
            niter.append(int(lines[5 * loop]))
            train_times.append(float(lines[5 * loop + 1]))
            objectives.append(float(lines[5 * loop + 2]))
            imbalance.append(float(lines[5 * loop + 3]))
            quant_time += float(lines[5 * segments + loop * 2])
            search_time += float(lines[5 * segments + loop * 2 + 1])
        with open(csv_name,'a') as fd:
            fd.write("{},{},{},{},{},{},{},{},{},{}\n".format(
                nlist,nprobe,topK,
